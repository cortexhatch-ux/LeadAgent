...
class ContextScraper:
    ...
    def auto_discovery(self, project_path: str):
        ...
        for root, dirs, files in os.walk(root_path):
            ...
            dirs[:] = [
                ...
                for d in dirs
                if not d.startswith(".")
                and d not in ("node_modules", "__pycache__", "venv", "leadagent")
            ]
    ...
    def scan_and_import(self, project_path: str, enabled_agents: List[str]):
        ...
        print("📄 Changes detected in context files. Starting ingestion...")
    ...
    def start_watcher(self, project_path: str, enabled_agents: List[str]):
        ...
        threading.Thread(
            target=_loop, daemon=True, name=f"Watcher-{project_path}"
        ).start()
    ...
    def _ingest_to_graph_via_cli(self, text: str, project_id: str):
        ...
        try:
            ...
            print(
                f"✅ Successfully updated {len(data.get('entities', []))} entities from context in {project_id}."
            )
        except Exception as e:
