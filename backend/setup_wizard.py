...
/_____/\\___/\\__,_/\\__,_/  /_/  |_|\\__, /\\___/_/ /_/\\__/  
    ...
    try:
        raw = input(f"{CYAN}Enable which agents?{RESET} {GRAY}(comma-separated, blank for all):{RESET} ").strip()
    except EOFError:
        raw = ""
    ...
    try:
        p_dir = input(f"{CYAN}Projects workspace directory{RESET} {GRAY}[{default_projects}]:{RESET} ").strip()
    except EOFError:
        p_dir = ""
...
class SetupWizard:
    ...
    def _build_chrome(self):
        ...
        tk.Label(self.sidebar, text="🧠  LeadAgent", bg=SIDEBAR, fg=WHITE,
                 font=(SANS, 12, "bold")).pack(pady=(26, 2))
        tk.Label(self.sidebar, text="Setup Wizard", bg=SIDEBAR, fg=MUTED,
                 font=(SANS, 9)).pack()
        ...
        self.btn_next = tk.Button(
            bar, text="Continue →", command=self._next,
            bg=WHITE, fg=SIDEBAR, activebackground=SIDEBAR, activeforeground=WHITE,
            relief="flat", bd=0, padx=28, pady=10,
            font=(SANS, FONT_SIZE_L, "bold"), cursor="hand2",
        )
        self.btn_next.bind("<Enter>", lambda e: self.btn_next.config(bg=SIDEBAR, fg=WHITE))
        self.btn_next.bind("<Leave>", lambda e: self.btn_next.config(bg=WHITE, fg=SIDEBAR))
        self.btn_next.pack(side="right", padx=20, pady=8)
        self.btn_back = tk.Button(
            bar, text="← Back", command=self._back,
            bg=CARD, fg=TEXT, activebackground=SIDEBAR, activeforeground=WHITE,
            relief="flat", bd=0, padx=22, pady=10,
            font=(SANS, FONT_SIZE_BASE, "bold"), cursor="hand2", state="disabled",
        )
        self.btn_back.bind("<Enter>", lambda e: self.btn_back.config(bg=SIDEBAR, fg=WHITE) if self.btn_back["state"] != "disabled" else None)
        self.btn_back.bind("<Leave>", lambda e: self.btn_back.config(bg=CARD, fg=TEXT) if self.btn_back["state"] != "disabled" else None)
        self.btn_back.pack(side="right", padx=0, pady=8)


        self.status_label = tk.Label(bar, text="", bg=SIDEBAR, fg=MUTED, font=(SANS, FONT_SIZE_BASE))
    ...
    def _render(self):
        ...
        self.btn_next.config(command=self._next, text="Continue →",
    ...
    def _redraw_sidebar(self):
        ...
        for i, (label, _) in enumerate(self.steps):
            ...
            tk.Label(row, text=dot, bg=SIDEBAR, fg=fc,
                     font=(SANS, FONT_SIZE_L, fw), width=3).pack(side="left", padx=(10, 6))
            tk.Label(row, text=label, bg=SIDEBAR, fg=fc,
                     font=(SANS, FONT_SIZE_BASE, fw), anchor="w").pack(side="left")
    ...
    def _welcome(self):
        ...
        tk.Label(f, text="Welcome to LeadAgent", bg=BG, fg=SIDEBAR,
                 font=(SANS, FONT_SIZE_XL, "bold")).pack()
        tk.Label(f,
                 text="Your personal AI routing layer.\n"
                      "Route every prompt to the best available model — automatically.",
                 bg=BG, fg=TEXT, font=(SANS, FONT_SIZE_L), justify="center").pack(pady=(16, 40))

        pills = tk.Frame(f, bg=BG)
        pills.pack()
        for icon, label in [("🔀", "Smart routing"), ("💾", "Graph memory"), ("⚡", "Zero API cost")]:
            p = tk.Frame(pills, bg=CARD, padx=22, pady=18)
            p.pack(side="left", padx=12)
            tk.Label(p, text=f"{icon}  {label}", bg=CARD, fg=TEXT,
                     font=(SANS, FONT_SIZE_BASE, "bold")).pack()
    ...
    def _select(self):
        f = self._content()
        tk.Label(f, text="AI Subscriptions",
                 bg=BG, fg=SIDEBAR, font=(SANS, FONT_SIZE_XL, "bold")).pack(anchor="w")
        tk.Label(f, text="LeadAgent routes only to models you enable below.",
                 bg=BG, fg=TEXT, font=(SANS, FONT_SIZE_L)).pack(anchor="w", pady=(6, 24))
        ...
        for key in AGENT_ORDER:
            ...
            tk.Label(card, text="●", bg=CARD, fg=spec.color,
                     font=(SANS, 24)).pack(side="left", padx=(0, 16))

            info = tk.Frame(card, bg=CARD)
            info.pack(side="left", fill="x", expand=True)
            tk.Label(info, text=spec.display, bg=CARD, fg=WHITE,
                     font=(SANS, FONT_SIZE_L, "bold"), anchor="w").pack(anchor="w")
            
            install_status = " •  installed" if is_installed(key) else ""
            status_color = GREEN if is_installed(key) else MUTED
            sub = tk.Label(info, text=spec.vendor, bg=CARD, fg=MUTED,
                           font=(SANS, FONT_SIZE_BASE), anchor="w")
            sub.pack(side="left")
            if install_status:
                tk.Label(info, text=install_status, bg=CARD, fg=GREEN,
                         font=(SANS, FONT_SIZE_BASE, "bold")).pack(side="left")

            if not spec.npm_pkg:
                tk.Label(card, text=spec.note or "Coming Soon",
                         bg=CARD, fg=YELLOW, font=(SANS, FONT_SIZE_BASE, "bold")).pack(side="right", padx=10)
            tk.Checkbutton(card, variable=var, bg=CARD, activebackground=CARD,
                           selectcolor=BG, cursor="hand2", font=(SANS, FONT_SIZE_L)).pack(side="right")
        ...
        def _continue():
            ...
            if not sel:
                ...
                return
            self.selected = sel
            installable = [k for k in AGENT_ORDER
                           if k in self.selected and AGENTS[k].npm_pkg]
            self.steps = (
    ...
    def _workspace(self):
        f = self._content()
        tk.Label(f, text="📂 Workspace Directory",
                 bg=BG, fg=SIDEBAR, font=(SANS, FONT_SIZE_XL, "bold")).pack(anchor="w")
        tk.Label(f, text="LeadAgent needs to know where your projects live for Auto-Discovery.",
                 bg=BG, fg=TEXT, font=(SANS, FONT_SIZE_L)).pack(anchor="w", pady=(6, 24))

        card = tk.Frame(f, bg=CARD, padx=22, pady=24)
        card.pack(fill="x")

        tk.Label(card, text="Absolute path to projects folder:",
                 bg=CARD, fg=TEXT, font=(SANS, FONT_SIZE_L, "bold")).pack(anchor="w")
        ...
        if os.path.exists(CONFIG_FILE):
            try:
                ...
                if prev: current_p = prev
            except: pass

        self.p_entry = tk.Entry(card, bg=CODE, fg=WHITE, insertbackground=WHITE,
                                font=(MONO, FONT_SIZE_XL, "bold"), relief="flat", bd=12)
        self.p_entry.pack(fill="x", pady=(16, 10))
        self.p_entry.insert(0, current_p)

        tk.Label(card, text="Example: /Users/username/Projects",
                 bg=CARD, fg=MUTED, font=(SANS, FONT_SIZE_BASE)).pack(anchor="w")

        def _save_and_next():
            p_path = self.p_entry.get().strip()
            if not p_path or not p_path.startswith("/"):
                self.status_label.config(text="Please provide an absolute path.", fg=RED)
                return
    ...
    def _agent(self, key: str):
        ...
        tk.Label(h, text="●", bg=BG, fg=spec.color,
                 font=(SANS, 20)).pack(side="left", padx=(0, 12))
        hh = tk.Frame(h, bg=BG)
        hh.pack(side="left")
        tk.Label(hh, text=spec.display, bg=BG, fg=SIDEBAR,
                 font=(SANS, FONT_SIZE_L, "bold"), anchor="w").pack(anchor="w")
        tk.Label(hh, text=f"Connect your {spec.vendor} subscription",
                 bg=BG, fg=TEXT, font=(SANS, FONT_SIZE_BASE), anchor="w").pack(anchor="w")
        ...
        card_state["install_card"] = self._cmd_card(
            ...
            note=None if installed else "Add to PATH: export PATH=\"$HOME/.leadagent/bin:$PATH\"",
        ...
        tk.Label(f, text="Run the commands in a terminal, then click Verify.",
                 bg=BG, fg=TEXT, font=(SANS, FONT_SIZE_BASE)).pack(anchor="w", pady=(16, 0))

    def _cmd_card(self, parent, step, done, done_label, cmd, note=None, verify=None):
        card = tk.Frame(parent, bg=CARD, padx=22, pady=18, highlightbackground="#E0E2EE", highlightthickness=1)
        card.pack(fill="x", pady=8)

        top = tk.Frame(card, bg=CARD)
        top.pack(fill="x")
        dot = tk.Label(top, text="✓" if done else "○", bg=CARD,
                       fg=GREEN if done else MUTED,
                       font=(SANS, FONT_SIZE_XL, "bold"), width=2)
        dot.pack(side="left")
        tk.Label(top, text=step, bg=CARD, fg=SIDEBAR,
                 font=(SANS, FONT_SIZE_L, "bold")).pack(side="left")
        status_lbl = tk.Label(top, text=done_label if done else "", bg=CARD,
                              fg=GREEN if done else MUTED, font=(SANS, FONT_SIZE_BASE, "bold"))
        status_lbl.pack(side="right")

        if cmd:
            cb = tk.Frame(card, bg=CODE, padx=16, pady=14)
            cb.pack(fill="x", pady=(12, 0))
            tk.Label(cb, text=cmd, bg=CODE, fg=WHITE, 
                     font=(MONO, FONT_SIZE_L, "bold"), anchor="w", justify="left").pack(anchor="w")
            if note:
                tk.Label(card, text=note, bg=CARD, fg=TEXT,
                         font=(SANS, FONT_SIZE_BASE)).pack(anchor="w", pady=(8, 0))
            btns = tk.Frame(card, bg=CARD)
            btns.pack(anchor="w", pady=(16, 0))
            
            # Action buttons - EXTREME CONTRAST
            self._btn(btns, "Copy", lambda c=cmd: self._copy(c)).pack(side="left", padx=(0, 12))
            self._btn(btns, "Open Terminal", lambda c=cmd: self._terminal(c)).pack(side="left", padx=(0, 12))
            if verify:
    ...
    def _verify(self, key: str, kind: str):
        installed = is_installed(key)
        if kind == "install":
            self._set_card(self._agent_cards[key].get("install_card"),
                           ok=installed,
                           ok_label="installed",
            ...
            authed = is_authenticated(key)
            self._set_card(self._agent_cards[key].get("login_card"),
                           ok=(authed is True),
                           ok_label="signed in",
    ...
    def _set_card(card_refs, ok: bool, ok_label: str, bad_label: str):
        ...
        card_refs["status"].config(text=ok_label if ok else bad_label,
    ...
    def _done(self):
        ...
        tk.Label(f, text="You're all set!", bg=BG, fg=SIDEBAR,
                 font=(SANS, 16, "bold")).pack()
        tk.Label(f, text="LeadAgent is configured and ready to route your prompts.",
                 bg=BG, fg=TEXT, font=(SANS, 10)).pack(pady=(4, 14))
        ...
        self.btn_next.config(text="Close", command=self.root.destroy,
                             bg=GREEN, fg=WHITE, activebackground="#4cae60")
    ...
    def _render_dashboard(self, health=None, backend_ok=False, starting=False, attempts=0):
        ...
        header.pack(fill="x")
        tk.Label(header, text="LeadAgent  Agent Dashboard", bg=ACCENT, fg=WHITE,
        ...
        tk.Label(header, text=status_dot, bg=ACCENT, fg=status_col,
        ...
        for key in AGENT_ORDER:
            ...
            row.pack(fill="x")
            tk.Label(row, text=dot, bg=CARD, fg=status_color,
                     font=(MONO, 13, "bold"), width=2).pack(side="left")
            tk.Label(row, text=key.upper(), bg=CARD, fg=spec.color,
                     font=(MONO, 10, "bold"), width=7).pack(side="left")
            tk.Label(row, text=status_text, bg=CARD, fg=status_color,
                     font=(SANS, 10)).pack(side="left")
            if usage:
                tk.Label(row, text=usage, bg=CARD, fg=MUTED,
                         font=(MONO, 9)).pack(side="left", padx=(6, 0))
        ...
        if not backend_ok and not starting:
            ...
            tk.Label(hint,
                     ...
                     bg=CARD, fg=MUTED, font=(SANS, 9)).pack(anchor="w")
